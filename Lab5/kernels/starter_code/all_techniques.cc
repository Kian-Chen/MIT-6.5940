    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                const uint8x16_t w0_low4bit = vandq_u8(w0, mask_low4bit);
                const uint8x16_t w0_upper4bit = vandq_u8(vshrq_n_u8(w0, 4), mask_low4bit)
                const int8x16_t w0_lowbit_s8 = vreinterpretq_s8_u8(w0_low4bit);
                const int8x16_t w0_upper4bit_s8 = vreinterpretq_s8_u8(vshrq_n_u8(w0, 4));

                const uint8x16_t w1_low4bit = vandq_u8(w1, mask_low4bit);
                const uint8x16_t w1_upper4bit = vandq_u8(vshrq_n_u8(w1, 4), mask_low4bit)
                const int8x16_t w1_lowbit_s8 = vreinterpretq_s8_u8(w1_low4bit);
                const int8x16_t w1_upper4bit_s8 = vreinterpretq_s8_u8(vshrq_n_u8(w1, 4));

                const uint8x16_t w2_low4bit = vandq_u8(w2, mask_low4bit);
                const uint8x16_t w2_upper4bit = vandq_u8(vshrq_n_u8(w2, 4), mask_low4bit)
                const int8x16_t w2_lowbit_s8 = vreinterpretq_s8_u8(w2_low4bit);
                const int8x16_t w2_upper4bit_s8 = vreinterpretq_s8_u8(vshrq_n_u8(w2, 4));

                const uint8x16_t w3_low4bit = vandq_u8(w3, mask_low4bit);
                const uint8x16_t w3_upper4bit = vandq_u8(vshrq_n_u8(w3, 4), mask_low4bit)
                const int8x16_t w3_lowbit_s8 = vreinterpretq_s8_u8(w3_low4bit);
                const int8x16_t w3_upper4bit_s8 = vreinterpretq_s8_u8(vshrq_n_u8(w3, 4));
                ...
                const int8x16_t w0_lower4bit_s8_central = vsubq_s8(w0_lowbit_s8, offsets);
                const int8x16_t w0_upper4bit_s8_central = vsubq_s8(w0_upper4bit_s8, offsets);

                const int8x16_t w1_lower4bit_s8_central = vsubq_s8(w1_lowbit_s8, offsets);
                const int8x16_t w1_upper4bit_s8_central = vsubq_s8(w1_upper4bit_s8, offsets);

                const int8x16_t w2_lower4bit_s8_central = vsubq_s8(w2_lowbit_s8, offsets);
                const int8x16_t w2_upper4bit_s8_central = vsubq_s8(w2_upper4bit_s8, offsets);

                const int8x16_t w3_lower4bit_s8_central = vsubq_s8(w3_lowbit_s8, offsets);
                const int8x16_t w3_upper4bit_s8_central = vsubq_s8(w3_upper4bit_s8, offsets);
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i w_low4bit = _mm256_and_si256(raw_w, lowMask);
                __m256i w_upper4bit = _mm256_and_si256(_mm256_srli_epi16(raw_w, 4), lowMask);

                __m256i w_next_low4bit = _mm256_and_si256(raw_w_next, lowMask);
                __m256i w_next_upper4bit = _mm256_and_si256(_mm256_srli_epi16(raw_w_next, 4), lowMask);
                ...
                w_0 = _mm256_sub_epi8(w_low4bit, zero_point);
                w_128 = _mm256_sub_epi8(w_upper4bit, zero_point);

                w_0_next = _mm256_sub_epi8(w_next_low4bit, zero_point);
                w_128_next = _mm256_sub_epi8(w_next_upper4bit, zero_point);
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    int column_per_thread = params->C.column / num_thread;
    for (int i = 0; i < num_thread; i++) { 
        threads_args[i].start_j = i * column_per_thread;
        threads_args[i].end_j = (i + 1) * column_per_thread;
